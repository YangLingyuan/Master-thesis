...
\begin{figure}[t]\label{fig:controller}
	\begin{center}
		\begin{tikzpicture}
			...
			\draw[myarrow] (-3, 2) |-(ICR.west) node [anchor=south east]{$\hat{\dot{\beta}},\hat{\beta},\hat{\dot{\phi}}$};
			\draw[myarrow] (-3, 0) --(FKAM.west) node [anchor=south east]{$\hat{\dot{\beta}},\hat{\beta},\hat{\dot{\phi}}$};
			\draw[myarrow] (-3, -2) |-(IKAM.west) node [anchor=south east]{$\dot{\xi},\ddot{\xi}$};
			\draw[myarrow] (-3, 2) |-(ICR.west) node [anchor=north east]{$\dot{\xi},\ddot{\xi}$};
